...
async function fetchMediumPosts() {
  try {
    console.log(`\n📡 Fetching Medium RSS from: ${MEDIUM_RSS_URL}`);
    ...
    posts.slice(0, 3).forEach((post, i) => {
      ...
      console.log(`      📅 ${new Date(post.pubDate).toLocaleDateString()}`);
      console.log(`      🏷️  ${post.categories.slice(0, 3).join(', ')}${post.categories.length > 3 ? '...' : ''}`);
    });

    if (posts.length === 0) {
      console.warn('\n⚠️  Warning: No blog posts found in RSS feed');
    ...
    console.log('⚠️  Created empty posts file to prevent build failure');
